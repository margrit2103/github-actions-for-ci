\echo Cost Centre test - post all cost-center relevant (and non-relevant) transactions.  
\echo This scripts sets up the entity as a cost-center entity at the beginning of the script
\echo ... and switches it off again at the end
\echo Script assumes that at least one cost center has been created (FUNCTION lysglb.init_dummy_cost_centre())

\echo Switch to cost-center entity
select lysglb.update_toggles_jdata( entity, toggleid,  jdata||jsonb_build_object( 'kosjn', 'D' ) ) FROM lysglb.toggles where toggleid = 'fixdata_0' and entity = 1;
\echo Link all service centres to cost_centers
WITH a AS (UPDATE lysglb.glist SET extra_int[3] = 1 WHERE ((glist.entity = 1) AND (glist.tipe = 17)) RETURNING 1 AS i )
SELECT sum(i) > 0 AS service_centres_set FROM a;
...
\echo Parameters: this test assumes that a goodx1 directory exists, and that at least one debtor has been created and is ready for posting. 
    ...
    SELECT set_config('test.cost_center', min( fdcode)::text, FALSE) <> '' as cost_centre FROM lysglb.v_cost_centre limit 1;
...
\echo Post creditor ledger invoice
    WITH _payload AS (
                                        ...
                                        <creditor_invoices>
                                            <creditor_invoice>
                                                ...
                                                <row>
                                                    ...
                                                    <cost_centre_integer>%5$s</cost_centre_integer>
                                                </row>
                                                <row>
                                                    <a_id_integer>2</a_id_integer>
        ...
        ', current_setting( 'test.period' )::INTEGER, 
           to_char( now(), 'yyyy-mm-dd' ), 
           current_setting( 'test.creditor' )::INTEGER , 
           'Ldgr:' || to_char( now(), 'yyyymmddhhMIss' ),
           current_setting('test.cost_center')
...
\echo Post creditor stock invoice: no cost centre required
    WITH _payload AS (
        SELECT unnest( xpath( '/table/feedback/summary/feedback_summary/text()', 
         tariffs.post_xml( format( '<table>
                                        <global_data>
...
\echo Post journal: Ledgers.  SCR1 no cost centre
                                                    ...
                                                    <journals>
                                                        <journal>
                                                            ...
                                                            <entry>
                                                                ...
                                                                <cost_centre_id>%3$s</cost_centre_id>
                                                                <description>TEST</description>
                                                                <jnl_action/>
                                                                <amount>12</amount>
                                                                <vat>1.57</vat>
                                                    ...
                                                    <journals>
                                                        <journal>
                                                            ...
                                                            <entry>
                                                                ...
                                                                <cost_centre_id>%5$s</cost_centre_id>
                                                                <description>TEST</description>
                                                                <jnl_action/>
                                                                <amount>500</amount>
                                                                <vat>65.22</vat>
                                                    ...
                                                    <journals>
                                                        <journal>
                                                            ...
                                                            <entry>
                                                                <a_id_integer>2</a_id_integer>
                                                                <pma_no_d>INC1</pma_no_d>
                                                                <cost_centre_id>%6$s</cost_centre_id>
...
\echo Post deposit to ledger: INC1 cost centre, SCR1 no cost centre
                                                ...
                                                <payment>
                                                    ...
                                                    <items>
                                                        <item>
                                                            ...
                                                            <cost_centre_id>%4$s</cost_centre_id>
                                                        </item>
                                                    </items>
                                                </payment>
                                            </payments>
    ...
    WITH _traninfo AS ( select docnr FROM lysglb.cashoop order by coid desc limit 1),
        _payload AS ( select format( ' <era>
                                    ...
                                    ',  current_setting( 'test.period' )::INTEGER, 
                                        to_char( now(), 'yyyy-mm-dd'), 
                                        docnr,
                                        current_setting('test.cost_center'))::XML as payload
...
\echo Post receipt (deposit) on debtor invoice, no cost center
...
\echo Reverse receipt (deposit) on debtor invoice: too much, no cost center
    WITH _inv AS ( select doknr as deposit_no, turnover_source_doc, reknr as _debtor, splitnr  FROM analysis.cf_contra order by uid desc limit 1 ),
         _payload as ( SELECT format(  '<era>
                                            <header>
                                                <a_id_integer>1</a_id_integer>
                                                <delete_temp_integer>0</delete_temp_integer>
...
\echo Add to receipt (deposit) on debtor invoice, no cost center
    WITH _inv AS ( select doknr as deposit_no, turnover_source_doc, reknr as _debtor, splitnr  FROM analysis.cf_contra order by uid desc limit 1 ),
         _payload as ( SELECT format(  '<era>
                                            <header>
                                                <a_id_integer>1</a_id_integer>
...
\echo Reverse payment (cheque) on creditor invoice
    WITH _inv AS ( select doknr as transaction_no, turnover_source_doc, reknr as _pma_no, splitnr  FROM analysis.cf_contra order by uid desc limit 1 ),
         _payload as ( SELECT format(  '<era>
                                            <header>
                                                <a_id_integer>1</a_id_integer>
                                                <delete_temp_integer>0</delete_temp_integer>
...
\echo Switch off cost-center entity
select lysglb.update_toggles_jdata( entity, toggleid,  jdata||jsonb_build_object( 'kosjn', 'N' ) ) FROM lysglb.toggles where toggleid = 'fixdata_0' and entity = 1;
\echo Unlink all service centres to cost_centers
WITH a AS (UPDATE lysglb.glist SET extra_int[3] = 0 WHERE ((glist.entity = 1) AND (glist.tipe = 17)) RETURNING 1 AS i )
SELECT sum(i) > 0 AS service_centres_reset FROM a;